...
module.exports = function(server, config) {
    ...
    var validate = function (username, password, callback) {
        User.findOne({email: username}, {_id:1, api_keys:1, password: 1, name: 1, hash: 1}, function(err, user) {
            if (err) {
                return callback(err);
            }
    ...
    routes.authorize = {
        ...
        validate: {
            payload: {
                ...
                password: Hapi.types.string().max(50).required()
            }
        }
    };
    ...
    routes.register = {
        ...
        validate: {
            payload: {
                email: Hapi.types.string().email().required(),
                password: Hapi.types.string().required().min(6).max(50),
