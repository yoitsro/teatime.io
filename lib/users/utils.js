...
var updateTeamOwnersWithNewUserInfo = function(user, cb) {
    Team.update({'owners._id': user._id}, { $set: { 'owners.$': user.desensitize() }}, function(err, numAffected) {
        if(err) {
            console.error(err);
        }
...
var updateTeamMembersWithNewUserInfo = function(user, cb) {
    Team.update({'members._id': user._id}, { $set: { 'members.$': user.desensitize() }}, function(err, numAffected) {
        if(err) {
            console.error(err);
        }
