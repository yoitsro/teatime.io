...
module.exports = function(server, config) {
    ...
    var updateSingleUser = function(request, reply) {
        ...
        if(request.payload.email) {
            ...
            updateQuery.hash = require('crypto').createHash('md5').update(request.payload.email).digest("hex");
        }
        ...
        if(request.payload.profileImage || request.payload.mugImage) {
            updateQuery.image = {};
        }

        if(request.payload.profileImage) {
            updateQuery.image.profile = request.payload.profileImage;
        }

        if(request.payload.mugImage) {
            updateQuery.image.mug = request.payload.mugImage;
        }
    ...
    routes.updateSingleUser = {
        ...
        validate: {
            ...
            payload: {
                ...
                password: Hapi.types.string().min(6).max(50),
                profileImage: Hapi.types.string().min(10).max(400),
                mugImage: Hapi.types.string().min(10).max(400)
            }
        }
    };
