...
module.exports = function(server, config) {
    ...
    var getOrders = function(request, reply) {
        ...
        TeaRound.findOne({group: request.params.id, 'members._id': user._id}, {orders: {$slice:[skip, limit +1]}}).exec(function(err, teaRound) {
            if(err) {
                return reply(err);
            }
    ...
    var getOrder = function(request, reply) {
        ...
        TeaRound.findOne({group: request.params.id, 'members._id': user._id}, {orders: 1}).exec(function(err, teaRound) {
            if(err) {
                return reply(err);
            }

            if(!teaRound.orders.id(request.params.orderId)) {
                return reply(Hapi.error.notFound("This tea round order does not exist"));
            }

            return reply(teaRound.orders.id(request.params.orderId));
        });
    };
    ...
    var createOrder = function(request, reply) {
        ...
        TeaRound.findOneAndUpdate({_id: request.params.roundId, group: request.params.id, 'members._id': user._id}, update, function(err, teaRound) {
            if(err) {
                return reply(err);
            }
            ...
            reply(order).code(201).header('Location', '/groups/' + request.params.id + '/tea-rounds/' + request.params.roundId + "/orders/" + order._id);
        });
    };
    ...
    var updateOrder = function(request, reply) {
        ...
        TeaRound.findOneAndUpdate({_id: request.params.roundId, group: request.params.id, 'orders.owner._id': user._id }, update).exec(function(err, teaRound) {
            if(err) {
                return reply(err);
            }
    ...
    var deleteOrder = function(request, reply) {
        ...
        TeaRound.findOne({_id: request.params.roundId, group: request.params.id, 'members._id': user._id}, {orders: 1}).exec(function(err, teaRound) {
            if(err) {
                return reply(err);
            }
