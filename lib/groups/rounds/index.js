...
module.exports = function(server, config) {
    ...
    var getRounds = function(request, reply) {
        ...
        Round.find({group: request.params.id, 'members._id': user._id}).skip(skip).limit(limit + 1).exec(function(err, rounds) {
            if(err) {
                return reply(err);
            }
    ...
    var getRound = function(request, reply) {
        ...
        Round.findOne({_id: request.params.roundId, group: request.params.id, 'members._id': user._id}).exec(function(err, round) {
            if(err) {
                return reply(err);
            }
