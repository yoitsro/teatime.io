...
exports.inviteRemainingUsers = function(users, group, inviter, callback) {
    // For each user, send them an invite
    Async.each(
        users,
        function(user, cb) {
            var message = Util.format("Hey there hot beverage drinker! You've been invited to join the group '%s' on teatime.io by %s.", group.name, inviter.name.first);
