        ...
        validate: {
            payload: {
                jsonData: Joi.object({
                    ...
                    members: Joi.array().includes(Joi.string().email(), Joi.string().regex(/^[0-9a-fA-F]{24}$/)).required(),
                    loc: Joi.array().length(2).includes(Joi.number().min(-180).max(180)),
    ...
    routes.updateTeam = {
        ...
        validate: {
            ...
            payload: {
                ...
                owners: Joi.array().includes(Joi.string().regex(/^[0-9a-fA-F]{24}$/)),
                loc: Joi.array().length(2).includes(Joi.number().min(-180).max(180)),
                searchable: Joi.boolean()
            }
        }
