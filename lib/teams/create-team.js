...
module.exports = internals = function(routes) {
    ...
    var createTeam = function(request, reply) {
        ...
        return internals.saveS3Image(request, team, function (err, request, team, url) {
            if (err) {
                return reply(err);
            }
            ...
            return team.save(function (err, team) {
                if (err) {
                    if(err.code === 11000) {
                        var error = Boom.badRequest("Uh oh! A team with that name already exists!");
                ...
                // Lookup the users 
                return  TeamUtils.lookupUsersByEmailOrId(request.payload.json.members, function (err, users, hashmap) {
                    if (err) {
                        return reply(Boom.internalError('Something went wrong', err));
                    }
                    ...
                    return TeamUtils.createRemainingUsers(Object.keys(hashmap), function (err, newUsers) {
                        if (err) {
                            console.error(err);
                        }
    ...
    var createSchema = {
        ...
        members: Joi.array().single().items(Joi.string().email()).min(1).description('The team members as an array of email addresses.'),
        loc: Joi.array().length(2).items(Joi.number().min(-180).max(180)).description('The team\'s location.'),
    ...
    internals.saveS3Image = function(request, team, done) {
        ...
        if (typeof request.payload.image === 'string') {
            ...
            return done(Boom.badRequest('Please use a valid image format.'));
        }
    ...
    var updateTeam = function(request, reply) {
        if(!request.payload) {
            return reply(Boom.badRequest('Please specify what you want to update'));
        }
    ...
    routes.updateTeam = {
        ...
        validate: {
            ...
            payload: {
                name: Joi.string().min(2).max(100).description('The team name.'),
                owners: Joi.array().single().items(Joi.string().regex(/^[0-9a-fA-F]{24}$/)).description('The team owners as an array of ids.'),
                loc: Joi.array().length(2).items(Joi.number().min(-180).max(180)).description('The team\'s location.'),
